...
class DKNN (torch.nn.Module):
    def __init__(self, k, tau=1.0, hard=False, method='deterministic', num_samples=-1, simple=False):
        super(DKNN, self).__init__()
        self.k = k
        self.soft_sort = SoftSort(tau=tau, hard=hard) if simple else NeuralSort(tau=tau, hard=hard)
        self.method = method
