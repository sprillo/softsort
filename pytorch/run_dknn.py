...
def experiment_id(dataset, k, tau, nloglr, method, simple):
    return 'dknn-resnet-%s-%s-k%d-t%d-b%d-%s' % (dataset, method, k, tau * 100, nloglr,
                                                 'simple' if simple else 'neural')


e_id = experiment_id(dataset, k, tau, args.nloglr, method, simple)


dknn_layer = DKNN(k, tau, method=method, num_samples=NUM_SAMPLES, simple=simple)
...
if dataset == 'mnist':
    ...
    h_phi = PreActResNet18(num_channels=3 if dataset == 'cifar10' else 1).to(gpu)
...
batched_query_train = split.get_train_loader(NUM_TRAIN_QUERIES, drop_last=True)
batched_neighbor_train = split.get_train_loader(NUM_TRAIN_NEIGHBORS, drop_last=True)
...
def train(epoch):
    ...
    to_average = []
    # train
    for query, candidates in zip(batched_query_train, loopy(batched_neighbor_train)):
        optimizer.zero_grad()
